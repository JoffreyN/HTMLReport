...
def _mk_test_name(name, value, index: int = 0):
    ...
    # 添加对字典数据的处置
    # 判断若是不是字典类型，条件就不知足；value不是字典类型则为假，就不会按原来拼接
    if not _is_trivial(value) and not isinstance(value, dict):
        return "{0}_{1}".format(name, index)

    # 若是数据是字典，则获取字典当中的title对应的值，加到测试用例名称中
    if isinstance(value, dict):
        try:
            # 取出title值
            value = value["desc"]  # 这个“case_name”要和测试数据的字段名一致
        except KeyError:
            return "{0}_{1}".format(name, index)
    ############## edit by zp end #####################
    try:
